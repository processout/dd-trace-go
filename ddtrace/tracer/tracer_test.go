...
func TestPushTrace(t *testing.T) {
	...
	t0 := <-tracer.traceBuffer
	assert.Equal(trace, t0)

	many := traceBufferSize + 2
	for i := 0; i < many; i++ {
		tracer.pushTrace(make([]*span, i))
	}
	assert.Len(tracer.traceBuffer, traceBufferSize)
...
func (t *dummyTransport) send(p *payload) (*http.Response, error) {
	traces, err := decode(p)
	if err != nil {
		return nil, err
	}
